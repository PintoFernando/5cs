...
class CroquisGaranteController extends Controller
{
	...
	public function create()
	{
		if (Session::get('id_persona_garante') == null) {
			...
			$cro = Croquis::where('id_persona', Session::get('id_persona_garante'))->count();
			if ($cro > 3) {
	...
	{ 
		...
		{
			$cro = Croquis::where('id_persona', Session::get('id_persona_garante'))->where('id_categoria_croquis',1)->count();
			if ($cro >=1){
			...
			}else
			{
				...
				$cro->id_persona =Session::get('id_persona_garante'); ;
...
{
	$cro1 = Croquis::where('id_persona', Session::get('id_persona_garante'))->where('id_categoria_croquis',2)->count();
	if ($cro1 >=1){
...
{
	$cro2 = Croquis::where('id_persona', Session::get('id_persona_garante'))->where('id_categoria_croquis',3)->count();
	if ($cro2 >=1){
...
{
	$cro2 = Croquis::where('id_persona', Session::get('id_persona_garante'))->where('id_categoria_croquis',4)->count();
	if ($cro2 >=1){
		alert()->info('Info', 'Ya registro sus datos de croquis de actividad económica.')->showConfirmButton();
		return redirect('oficial/a_garantes/croquis/');
	}else{
