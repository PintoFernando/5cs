...
class FormularioCodeudorController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Session::get('id_persona_codeudor') == null) {
            ...
            $if_exist_credito = Credito::where('id_persona', Session::get('id_persona_codeudor'))->count();
            ...
            $if_exist_persona = Persona::where('id_persona', Session::get('id_persona_codeudor'))->count();
            ...
            ->where('persona.id_persona', $this->id_persona_codeudor)
            ->get();

            //dd($persona);
            //referencia solicitante
            $if_exist_referencia=ReferenciaSolicitante::where('id_persona',Session::get('id_persona_codeudor'))->count();
       ...
       $if_exist_direccion = Direccion::where('id_persona', Session::get('id_persona_codeudor'))->count();
       ...
       $if_exist_datos_empresa = DatosEmpresa::where('id_persona', Session::get('id_persona_codeudor'))->count();
       ...
       $if_exist_actividad_eco = ActividadEconomica::where('id_persona', Session::get('id_persona_codeudor'))->count();
       $actividad_eco = ActividadEconomica::where('id_persona', Session::get('id_persona_codeudor'))->get();

       $if_exists_conyugue = Conyugue::where('id_persona', Session::get('id_persona_codeudor'))->count();
    ...
    $if_exist_deposito = DepositoBancario::where('id_persona', Session::get('id_persona_codeudor'))->count();
    ...
    $if_exist_inversiones = InversionesFinancieras::where('id_persona', Session::get('id_persona_codeudor'))->count();
    $inversiones = InversionesFinancieras::where('id_persona', Session::get('id_persona_codeudor'))->get();
    $if_exist_cuentas_cobrar = CuentasDocumentosCobrar::where('id_persona', Session::get('id_persona_codeudor'))->count();
    $cuentas_cobrar = CuentasDocumentosCobrar::where('id_persona', Session::get('id_persona_codeudor'))->get();
    $if_exist_mercaderia = InventarioMercaderia::where('id_persona', Session::get('id_persona_codeudor'))->count();
    $mercaderia = InventarioMercaderia::where('id_persona', Session::get('id_persona_codeudor'))->get();
    $if_exist_maquinaria = MaquinariaEquipo::where('id_persona', Session::get('id_persona_codeudor'))->count();
    $maquinaria = MaquinariaEquipo::where('id_persona', Session::get('id_persona_codeudor'))->get();
    $if_exist_bienes = BienesHogar::where('id_persona', Session::get('id_persona_codeudor'))->count();
    $bienes = BienesHogar::where('id_persona', Session::get('id_persona_codeudor'))->get();
    $if_exist_inmuebles = Inmueble::where('id_persona', Session::get('id_persona_codeudor'))->count();
    $inmuebles = Inmueble::where('id_persona', Session::get('id_persona_codeudor'))->get();
    $if_exist_vehiculo = Vehiculo::where('id_persona', Session::get('id_persona_codeudor'))->count();
    $vehiculo = Vehiculo::where('id_persona', Session::get('id_persona_codeudor'))->get();
    $if_exist_otros = OtroActivo::where('id_persona', Session::get('id_persona_codeudor'))->count();
    $otros = OtroActivo::where('id_persona', Session::get('id_persona_codeudor'))->get();
    $if_exist_prestamo = PrestamoBancario::where('id_persona', Session::get('id_persona_codeudor'))->count();
    ...
    $if_exist_cuentas = CuentasPorPagar::where('id_persona', Session::get('id_persona_codeudor'))->count();
    $cuentas = CuentasPorPagar::where('id_persona', Session::get('id_persona_codeudor'))->get();
// I CALCULO DE PASIVOS
    $total_efectivos_caja = DB::table('efectivos_caja')->where('id_persona', Session::get('id_persona_codeudor'))->sum('caja');
    $total_depositos_bancarios = DB::table('deposito_bancario')->where('id_persona', Session::get('id_persona_codeudor'))->sum('saldo');
    $total_cuentas_cobrar = DB::table('cuentas_documentos_cobrar')->where('id_persona', Session::get('id_persona_codeudor'))->sum('saldo');
    $total_inversiones = DB::table('inversiones_financieras')->where('id_persona', Session::get('id_persona_codeudor'))->sum('valor_mercado');
    $total_maquinaria = DB::table('maquinaria_equipo')->where('id_persona', Session::get('id_persona_codeudor'))->sum('total');
    $total_mercaderia_inventarios = DB::table('inventario_mercaderia')->where('id_persona', Session::get('id_persona_codeudor'))->sum('precio_unitario');
    $total_propiedades = DB::table('inmueble')->where('id_persona', Session::get('id_persona_codeudor'))->sum('valor');
            $total_vehiculos = DB::table('vehiculo')->where('id_persona', Session::get('id_persona_codeudor'))->sum('valor'); //sumatoria del total de pasivos
            $total_bienes_hogar = DB::table('bienes_hogar')->where('id_persona', Session::get('id_persona_codeudor'))->sum('valor'); //sumatoria del total de pasivos
            $total_otros_activos = DB::table('otros_activos')->where('id_persona', Session::get('id_persona_codeudor'))->sum('total') + $total_bienes_hogar; //sumatoria del total de pasivos
            ...
            //CALCULO DE PASIVOS
            $total_prestamos_bancarios = DB::table('prestamo_bancario')->where('id_persona', Session::get('id_persona_codeudor'))->sum('saldo');
            $total_cuentas_por_pagar_saldo = DB::table('cuentas_por_pagar')->where('id_persona', Session::get('id_persona_codeudor'))->sum('saldo');
...
//total suma de pasivos y patrimonio
            ...
            $if_exist_ingresos = IngresoMensual::where('id_persona', Session::get('id_persona_codeudor'))->count();
            $ingresos = IngresoMensual::where('id_persona', Session::get('id_persona_codeudor'))->get();

            $if_exist_croquis_direccion = Croquis::where('id_persona', Session::get('id_persona_codeudor'))->where('id_categoria_croquis',1)->count();

            $croquis_direccion = Croquis::where('id_persona', Session::get('id_persona_codeudor'))->where('id_categoria_croquis',1)->first();
            

            $if_exist_croquis_trabajo = Croquis::where('id_persona', Session::get('id_persona_codeudor'))->where('id_categoria_croquis',3)->count();
            $croquis_trabajo = Croquis::where('id_persona', Session::get('id_persona_codeudor'))->where('id_categoria_croquis',3)->first();

            $if_exist_croquis_empresa = Croquis::where('id_persona', Session::get('id_persona_codeudor'))->where('id_categoria_croquis',3)->count();
            $croquis_empresa = Croquis::where('id_persona', Session::get('id_persona_codeudor'))->where('id_categoria_croquis',3)->first();


            $if_exist_ventas = Ventas::where('id_persona', Session::get('id_persona_codeudor'))->count();

            $ventas = Ventas::where('id_persona', Session::get('id_persona_codeudor'))->get();

            $if_exist_comercializacion = VentaComercializacionProducto::where('id_persona', Session::get('id_persona_codeudor'))->count();

            $comercializacion = VentaComercializacionProducto::where('id_persona', Session::get('id_persona_codeudor'))->get();

            $if_exist_obra = ManoObraMensual::where('id_persona', Session::get('id_persona_codeudor'))->count();
            $obra = ManoObraMensual::where('id_persona', Session::get('id_persona_codeudor'))->get();

            $if_exist_familiares = GastosFamiliares::where('id_persona', Session::get('id_persona_codeudor'))->count();
            $familiares = GastosFamiliares::where('id_persona', Session::get('id_persona_codeudor'))->get();

            $if_exist_operativos = GastosOperativosComercializacion::where('id_persona', Session::get('id_persona_codeudor'))->count();
            $operativos = GastosOperativosComercializacion::where('id_persona', Session::get('id_persona_codeudor'))->get();

            $ingreso_promedio_prestatario = DB::table('ingreso_mensual')->where('id_persona', Session::get('id_persona_codeudor'))->avg('prestatario');
            $ingreso_promedio_conyugue = DB::table('ingreso_mensual')->where('id_persona', Session::get('id_persona_codeudor'))->avg('conyugue');
            $ingreso_promedio_otros = DB::table('ingreso_mensual')->where('id_persona', Session::get('id_persona_codeudor'))->avg('otros');
            $ingreso_promedio_codeudores = DB::table('ingreso_mensual')->where('id_persona', Session::get('id_persona_codeudor'))->avg('codeudores');
            $ingreso_promedio_total = DB::table('ingreso_mensual')->where('id_persona', Session::get('id_persona_codeudor'))->avg('total_ingreso');

            $v_precio_total = DB::table('venta_comercializacion_productos')->where('id_persona', Session::get('id_persona_codeudor'))->sum('v_precio_total');
            $c_costo_total = DB::table('venta_comercializacion_productos')->where('id_persona', Session::get('id_persona_codeudor'))->sum('c_costo_total');

            $u_utilidad = DB::table('venta_comercializacion_productos')->where('id_persona', Session::get('id_persona_codeudor'))->sum('utilidad');
            ...
            $calculo = $utilidad_operativa * $this->capacidadPago(Session::get('id_persona_codeudor'));

            $total_cuentas_por_pagar = DB::table('cuentas_por_pagar')->where('id_persona', Session::get('id_persona_codeudor'))->sum('cuota_mensual');
            ...
            //margen de ahorro
            $total_prestamos = DB::table('prestamo_bancario')->where('id_persona', Session::get('id_persona_codeudor'))->sum('importe_ultimo_pago');
            ...
            ->with('porcentaje_capacidad_pago', $this->capacidadPago(Session::get('id_persona_codeudor')))
