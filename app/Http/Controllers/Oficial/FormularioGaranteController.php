...
class FormularioGaranteController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Session::get('id_persona_garante') == null) {
            ...
            $if_exist_credito = Credito::where('id_persona', Session::get('id_persona_garante'))->count();
            ...
            $if_exist_persona = Persona::where('id_persona', Session::get('id_persona_garante'))->count();
            $personas = DB::table('persona')
                ...
                ->where('persona.id_persona', $this->id_persona_garante)
                ->get();

            //dd($persona);
            //referencia solicitante
            $if_exist_referencia = ReferenciaSolicitante::where('id_persona', Session::get('id_persona_garante'))->count();
            ...
            $if_exist_direccion = Direccion::where('id_persona', Session::get('id_persona_garante'))->count();
            ...
            $if_exist_datos_empresa = DatosEmpresa::where('id_persona', Session::get('id_persona_garante'))->count();
            ...
            $if_exist_actividad_eco = ActividadEconomica::where('id_persona', Session::get('id_persona_garante'))->count();
            $actividad_eco = ActividadEconomica::where('id_persona', Session::get('id_persona_garante'))->get();

            $if_exists_conyugue = Conyugue::where('id_persona', Session::get('id_persona_garante'))->count();
            ...
            $if_exist_deposito = DepositoBancario::where('id_persona', Session::get('id_persona_garante'))->count();
            ...
            $if_exist_inversiones = InversionesFinancieras::where('id_persona', Session::get('id_persona_garante'))->count();
            $inversiones = InversionesFinancieras::where('id_persona', Session::get('id_persona_garante'))->get();
            $if_exist_cuentas_cobrar = CuentasDocumentosCobrar::where('id_persona', Session::get('id_persona_garante'))->count();
            $cuentas_cobrar = CuentasDocumentosCobrar::where('id_persona', Session::get('id_persona_garante'))->get();
            $if_exist_mercaderia = InventarioMercaderia::where('id_persona', Session::get('id_persona_garante'))->count();
            $mercaderia = InventarioMercaderia::where('id_persona', Session::get('id_persona_garante'))->get();
            $if_exist_maquinaria = MaquinariaEquipo::where('id_persona', Session::get('id_persona_garante'))->count();
            $maquinaria = MaquinariaEquipo::where('id_persona', Session::get('id_persona_garante'))->get();
            $if_exist_bienes = BienesHogar::where('id_persona', Session::get('id_persona_garante'))->count();
            $bienes = BienesHogar::where('id_persona', Session::get('id_persona_garante'))->get();
            $if_exist_inmuebles = Inmueble::where('id_persona', Session::get('id_persona_garante'))->count();
            $inmuebles = Inmueble::where('id_persona', Session::get('id_persona_garante'))->get();
            $if_exist_vehiculo = Vehiculo::where('id_persona', Session::get('id_persona_garante'))->count();
            $vehiculo = Vehiculo::where('id_persona', Session::get('id_persona_garante'))->get();
            $if_exist_otros = OtroActivo::where('id_persona', Session::get('id_persona_garante'))->count();
            $otros = OtroActivo::where('id_persona', Session::get('id_persona_garante'))->get();
            $if_exist_prestamo = PrestamoBancario::where('id_persona', Session::get('id_persona_garante'))->count();
            ...
            $if_exist_cuentas = CuentasPorPagar::where('id_persona', Session::get('id_persona_garante'))->count();
            $cuentas = CuentasPorPagar::where('id_persona', Session::get('id_persona_garante'))->get();
// I CALCULO DE PASIVOS
            $total_efectivos_caja = DB::table('efectivos_caja')->where('id_persona', Session::get('id_persona_garante'))->sum('caja');
            $total_depositos_bancarios = DB::table('deposito_bancario')->where('id_persona', Session::get('id_persona_garante'))->sum('saldo');
            $total_cuentas_cobrar = DB::table('cuentas_documentos_cobrar')->where('id_persona', Session::get('id_persona_garante'))->sum('saldo');
            $total_inversiones = DB::table('inversiones_financieras')->where('id_persona', Session::get('id_persona_garante'))->sum('valor_mercado');
            $total_maquinaria = DB::table('maquinaria_equipo')->where('id_persona', Session::get('id_persona_garante'))->sum('total');
            $total_mercaderia_inventarios = DB::table('inventario_mercaderia')->where('id_persona', Session::get('id_persona_garante'))->sum('precio_unitario');
            $total_propiedades = DB::table('inmueble')->where('id_persona', Session::get('id_persona_garante'))->sum('valor');
            $total_vehiculos = DB::table('vehiculo')->where('id_persona', Session::get('id_persona_garante'))->sum('valor'); //sumatoria del total de pasivos
            $total_bienes_hogar = DB::table('bienes_hogar')->where('id_persona', Session::get('id_persona_garante'))->sum('valor'); //sumatoria del total de pasivos
            $total_otros_activos = DB::table('otros_activos')->where('id_persona', Session::get('id_persona_garante'))->sum('total') + $total_bienes_hogar; //sumatoria del total de pasivos
            ...
            //CALCULO DE PASIVOS
            $total_prestamos_bancarios = DB::table('prestamo_bancario')->where('id_persona', Session::get('id_persona_garante'))->sum('saldo');
            $total_cuentas_por_pagar_saldo = DB::table('cuentas_por_pagar')->where('id_persona', Session::get('id_persona_garante'))->sum('saldo');
...
//total suma de pasivos y patrimonio
            ...
            $if_exist_ingresos = IngresoMensual::where('id_persona', Session::get('id_persona_garante'))->count();
            $ingresos = IngresoMensual::where('id_persona', Session::get('id_persona_garante'))->get();

            $if_exist_croquis_direccion = Croquis::where('id_persona', Session::get('id_persona_garante'))->where('id_categoria_croquis', 1)->count();

            $croquis_direccion = Croquis::where('id_persona', Session::get('id_persona_garante'))->where('id_categoria_croquis', 1)->first();

            $if_exist_croquis_trabajo = Croquis::where('id_persona', Session::get('id_persona_garante'))->where('id_categoria_croquis', 3)->count();
            $croquis_trabajo = Croquis::where('id_persona', Session::get('id_persona_garante'))->where('id_categoria_croquis', 3)->first();

            $if_exist_croquis_empresa = Croquis::where('id_persona', Session::get('id_persona_garante'))->where('id_categoria_croquis', 2)->count();
            $croquis_empresa = Croquis::where('id_persona', Session::get('id_persona_garante'))->where('id_categoria_croquis', 2)->first();

            $if_exist_ventas = Ventas::where('id_persona', Session::get('id_persona_garante'))->count();

            $ventas = Ventas::where('id_persona', Session::get('id_persona_garante'))->get();

            $if_exist_comercializacion = VentaComercializacionProducto::where('id_persona', Session::get('id_persona_garante'))->count();

            $comercializacion = VentaComercializacionProducto::where('id_persona', Session::get('id_persona_garante'))->get();

            $if_exist_obra = ManoObraMensual::where('id_persona', Session::get('id_persona_garante'))->count();
            $obra = ManoObraMensual::where('id_persona', Session::get('id_persona_garante'))->get();

            $if_exist_familiares = GastosFamiliares::where('id_persona', Session::get('id_persona_garante'))->count();
            $familiares = GastosFamiliares::where('id_persona', Session::get('id_persona_garante'))->get();

            $if_exist_operativos = GastosOperativosComercializacion::where('id_persona', Session::get('id_persona_garante'))->count();
            $operativos = GastosOperativosComercializacion::where('id_persona', Session::get('id_persona_garante'))->get();

            $ingreso_promedio_prestatario = DB::table('ingreso_mensual')->where('id_persona', Session::get('id_persona_garante'))->avg('prestatario');
            $ingreso_promedio_conyugue = DB::table('ingreso_mensual')->where('id_persona', Session::get('id_persona_garante'))->avg('conyugue');
            $ingreso_promedio_otros = DB::table('ingreso_mensual')->where('id_persona', Session::get('id_persona_garante'))->avg('otros');
            $ingreso_promedio_codeudores = DB::table('ingreso_mensual')->where('id_persona', Session::get('id_persona_garante'))->avg('codeudores');
            $ingreso_promedio_total = DB::table('ingreso_mensual')->where('id_persona', Session::get('id_persona_garante'))->avg('total_ingreso');

            $v_precio_total = DB::table('venta_comercializacion_productos')->where('id_persona', Session::get('id_persona_garante'))->sum('v_precio_total');
            $c_costo_total = DB::table('venta_comercializacion_productos')->where('id_persona', Session::get('id_persona_garante'))->sum('c_costo_total');

            $u_utilidad = DB::table('venta_comercializacion_productos')->where('id_persona', Session::get('id_persona_garante'))->sum('utilidad');
            ...
            $calculo = $utilidad_operativa * $this->capacidadPago(Session::get('id_persona_garante'));

            $total_cuentas_por_pagar = DB::table('cuentas_por_pagar')->where('id_persona', Session::get('id_persona_garante'))->sum('cuota_mensual');
            ...
            //margen de ahorro
            $total_prestamos = DB::table('prestamo_bancario')->where('id_persona', Session::get('id_persona_garante'))->sum('importe_ultimo_pago');
            ...
            return view('oficial.a_garantes.formulario.index')
                ...
                ->with('porcentaje_capacidad_pago', $this->capacidadPago(Session::get('id_persona_garante')))
